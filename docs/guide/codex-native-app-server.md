# Codex App-Server 实现原理

本文记录当前 HAPI 对齐 Codex 原生 app-server 的实现方式，以及 native/tmux 退役后的边界。

当前基线：Codex CLI `0.122.0`，协议类型由本机 `codex app-server generate-ts --experimental` 生成。

## 目标

HAPI 现在只有一条 Codex 主路线：

1. **Codex app-server 路线**：使用 Codex CLI 原生 app-server 协议访问 thread、turn、review、plugin、MCP、memory 等能力。

native/tmux create、attach、resume、open 已退役。HAPI 不再把 tmux 作为 Codex 打开或恢复路径，也不再把 transcript-only 历史包装成可打开的 HAPI session。
...
2. **Review 展示**：`/review` 成功后保存 `reviewThreadId`，再通过 `thread/turns/list` 读取最近 turns，展示 review 输出、文件变更、命令和 MCP/tool 行。
...
`GET /api/codex-sessions` 会合并两类来源：

1. **已 attached 的 HAPI session**
   - `session.metadata.flavor === "codex"`
   - 有 `metadata.codexSessionId`
2. **app-server thread/list**
   - 优先从在线 Codex app-server session 调 `thread/list`
   - 没有现成 session 时，从在线 HAPI runner 的 machine-scoped app-server catalog 调 `thread/list`
...
相同 `codexSessionId` 的条目会合并，避免同一个 Codex thread 在列表里出现多次。transcript-only 历史不再进入这个列表；没有在线 app-server 来源时，HAPI 不会用 tmux/native `codex resume` 兜底打开。
...
remote Codex 聊天页现在优先走 app-server：

```text
thread/turns/list({ threadId, limit: 2, sortDirection: "desc" })
  -> recent turns/items
  -> codexThreadToMessages(...)
  -> HappyThread render
```

点击“加载更早”时，Web 会增加 `thread/turns/list` 的 `limit`，继续读取更早 turns。Workspace summary 的 `thread/read` 使用 `includeTurns: false`，只读取 thread 元信息，避免大 thread 首屏拉整条历史。

生成的 message id 稳定使用 `codex:${threadId}:${turnId}:${itemId || index}`。这些 message 不写回 HAPI message store，避免 app-server read 和 HAPI 实时消息产生重复历史。

以下场景才回落到 HAPI messages store：

- app-server read 失败
- session offline / inactive
- metadata 没有 `codexSessionId`

这个 fallback 只用于 HAPI 自己已经持久化过的消息，不再扫描 `~/.codex/sessions/**/*.jsonl`，也不再把 Codex Desktop / VS Code 的本机 transcript 包装成可打开 session。
...
HAPI 不再支持 native/tmux 作为产品路径：

```text
Codex 正常路径：app-server thread/read/resume/actions
Codex 历史来源：app-server thread/list
旧 tmux session：retired-native-tmux namespace，不可打开
```

具体行为：

- `/api/native-sessions/*` 返回 `410 Gone`。
- `/api/codex-sessions/open` 不再调用 native `codex resume`。
- `GET /api/codex-sessions` 不再暴露 transcript-only placeholder。
- retired native session 对普通 session API 表现为 `404 Session not found`。
- Hub 不再启动 native leadership，也不再恢复 tmux tracker。
...
- `GET /api/codex-sessions` 合并 attached 和 app-server thread/list。
- unattached app-server thread 不再被过滤，可以作为 `codex:` placeholder 打开。
- inactive session 不再灰显；只有 archived session 灰显。
- 聊天页 remote Codex 主路径优先用 `thread/turns/list` 分页渲染历史；HAPI message store 只做 fallback。
...
- native/tmux API 已退役，旧 session 隔离到 retired namespace。
...
bun run test:hub -- codexSessions.test.ts nativeSessions.test.ts sessions.test.ts
