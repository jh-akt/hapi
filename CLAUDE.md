...
- Treat commit messages, PR titles, PR bodies, release notes, and public issue text as public records.
- Do not include access tokens, session IDs that could identify a private user/session, tunnel URLs, private hostnames, machine names, personal paths, screenshots, log excerpts, diagnostic archive names, database dumps, or raw user prompts unless the user explicitly asks for that exact detail.
- Prefer behavior-focused commit messages such as `Add Codex app-server thread actions`; avoid messages that mention private domains, local incidents, production outage details, or user-specific debugging context.
- Before staging or committing, run `git status --short` and check that generated diagnostics, `.env*`, settings, local databases, logs, screenshots, and archives are not included by accident.
- If evidence from a private environment is useful, summarize it generically and redact sensitive values.
...
- Test framework: Vitest / Bun test via `bun run test`
- Test files: `*.test.ts` next to source
- Run: `bun run test` from root, or package-local test scripts as needed
